...
const User = ({ propUser, onEdit, onDelete }) => {
    ...
    return (
        <tr
            ...
            <td className={`px-6 py-4 text-sm font-semibold text-center max-w-[100px] wrap-break-words whitespace-normal rounded-full ${roleClass}`}>{propUser.maLoaiNguoiDung}</td>
            <td className="px-6 py-4 text-center flex justify-center items-center gap-2">
                <button onClick={() => onEdit && onEdit(propUser)} className="text-indigo-600 hover:text-indigo-900 px-3 py-1 rounded-md bg-gray-100 hover:bg-indigo-50 transition font-medium">
                    <i className="fi fi-rr-edit"></i>
                </button>
                <button onClick={() => onDelete && onDelete(propUser.taiKhoan)} className="text-red-600 hover:text-red-900 px-3 py-1 rounded-md bg-gray-100 hover:bg-red-50 transition font-medium">
